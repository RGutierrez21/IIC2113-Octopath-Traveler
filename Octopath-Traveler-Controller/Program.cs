...
string travelersPath = "data/characters.json";
string beastsPath = "data/enemies.json";
TeamsFileReader lector = new TeamsFileReader();
TeamParser traductor = new TeamParser();
CharacterRepository charRepo = new CharacterRepository(travelersPath);
BeastRepository beastRepo = new BeastRepository(beastsPath);
GameCatalogs catalogs = new GameCatalogs(charRepo, beastRepo);
TeamBuilderService teamBuilder = new TeamBuilderService(lector, traductor, catalogs);
AnnounceTestCase(test);

var view = View.BuildManualTestingView(test);
var game = new Game(view, teamsFolder, teamBuilder);
game.Play();
